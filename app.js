...
(function () {
  ...
  let companionListViewport = null;
  let overlayClipRect = null;
  let uidCounter = 0;
...
const COMPANION_ENCOUNTER_CHANCE = 0.7;
  ...
  const SHOP_CATALOG = [
    ...
    { id: "weapon-parchment-book", category: "item", name: "羊皮书", price: 40, rarity: "廉价", description: "不知名小说，书虫必备。", unlockValue: "羊皮书", shopVisible: true, purchaseMode: "gold" },
    { id: "weapon-red-book", category: "item", name: "小红书", price: 40, rarity: "廉价", description: "红色的封面，内容不详。", unlockValue: "小红书", shopVisible: true, purchaseMode: "gold" },
  ...
  function inferCompanionGiftCountFromLogs(logs) {
    return (Array.isArray(logs) ? logs : []).reduce(function (total, entry) {
      const message = entry && typeof entry.message === "string" ? entry.message : "";
      return message.indexOf("你给“") >= 0 && message.indexOf("”送出了“") >= 0 ? total + 1 : total;
    }, 0);
  }
  ...
  function normalizeState(input) {
    ...
    if (stateValue.companionGiftCount == null) stateValue.companionGiftCount = inferCompanionGiftCountFromLogs(stateValue.logs);
    stateValue.companionGiftCount = Math.max(0, Math.floor(Number(stateValue.companionGiftCount) || 0));
    ...
    if (!stateValue.migrations.companionZombieRenameV1) {
      const oldCompanionId = "zoobie";
      const newCompanionId = "zombie";
      if (!stateValue.companions || typeof stateValue.companions !== "object") stateValue.companions = {};
      if (stateValue.companions[oldCompanionId]) {
        stateValue.companions[newCompanionId] = Object.assign(
          {},
          stateValue.companions[newCompanionId] && typeof stateValue.companions[newCompanionId] === "object" ? stateValue.companions[newCompanionId] : {},
          stateValue.companions[oldCompanionId]
        );
        delete stateValue.companions[oldCompanionId];
      }
      if (stateValue.selectedCompanionId === oldCompanionId) {
        stateValue.selectedCompanionId = newCompanionId;
      }
      if (stateValue.adventure && stateValue.adventure.activeTrip && stateValue.adventure.activeTrip.encounterCompanionId === oldCompanionId) {
        stateValue.adventure.activeTrip.encounterCompanionId = newCompanionId;
      }
      if (stateValue.modal && stateValue.modal.companionId === oldCompanionId) {
        stateValue.modal.companionId = newCompanionId;
      }
      if (stateValue.modal && stateValue.modal.type === "codex" && stateValue.modal.selectedId === "companion:" + oldCompanionId) {
        stateValue.modal.selectedId = "companion:" + newCompanionId;
      }
      if (stateValue.codex && stateValue.codex.items && typeof stateValue.codex.items === "object") {
        const oldCodexId = "companion:" + oldCompanionId;
        const newCodexId = "companion:" + newCompanionId;
        if (stateValue.codex.items[oldCodexId]) {
          stateValue.codex.items[newCodexId] = Object.assign({}, stateValue.codex.items[oldCodexId], stateValue.codex.items[newCodexId] || {});
          stateValue.codex.items[newCodexId].id = newCodexId;
          stateValue.codex.items[newCodexId].key = newCompanionId;
          stateValue.codex.items[newCodexId].name = "Zombie";
          delete stateValue.codex.items[oldCodexId];
        }
      }
      if (stateValue.codex && Array.isArray(stateValue.codex.recentIds)) {
        stateValue.codex.recentIds = stateValue.codex.recentIds.map(function (id) {
          return id === "companion:" + oldCompanionId ? "companion:" + newCompanionId : id;
        });
      }
    ...
    stateValue.companionListScroll = Math.max(0, Number(stateValue.companionListScroll) || 0);
  ...
  if (syncSocialAchievements({ silent: true })) saveState();
      ...
      importFileInput.onchange = function (event) {
        ...
        reader.onload = function () {
          try {
            ...
            state = normalized;
            applyUiTheme(state.uiTheme);
            syncSocialAchievements({ silent: true });
  ...
  function companionById(companionId) {
    return COMPANIONS.find(function (companion) { return companion.id === companionId; }) || null;
  }
  ...
  function activeCompanionRecord(companionId) {
    ...
    if (!state.companions[companionId] || typeof state.companions[companionId] !== "object") {
      state.companions[companionId] = createDefaultCompanionRecord();
    }
    return state.companions[companionId];
  }
  ...
  function unlockedCompanionCount(targetState) {
    ...
    const companionsState = sourceState && sourceState.companions && typeof sourceState.companions === "object" ? sourceState.companions : {};
    return COMPANIONS.reduce(function (total, companion) {
      const record = companionsState[companion.id];
      return total + (record && record.unlocked ? 1 : 0);
    }, 0);
  }

  function companionsAtLeastAffectionLevel(level, targetState) {
  ...
  function activeHatchedPetCount(targetState) {
    const sourceState = targetState || state;
    const currentPets = Array.isArray(sourceState && sourceState.pets) ? sourceState.pets : [];
    const currentHatchedCount = currentPets.reduce(function (total, pet) {
      return total + (pet && !pet.isEgg ? 1 : 0);
    }, 0);
    return Math.max(currentHatchedCount, Math.max(0, Number(sourceState && sourceState.hatchedPetCount) || 0));
  }

  function hasAdultPet(targetState) {
    const sourceState = targetState || state;
    const currentPets = Array.isArray(sourceState && sourceState.pets) ? sourceState.pets : [];
    return currentPets.some(function (pet) {
  ...
  function companionPortraitSceneName(companion) {
    if (!companion) return "草地小径";
    const portraitScene = typeof companion.portraitScene === "string" ? companion.portraitScene.trim() : "";
    if (portraitScene) return portraitScene;
    const appearanceScene = companion.appearance && typeof companion.appearance.background === "string"
      ? companion.appearance.background.trim()
      : "";
    if (appearanceScene) return appearanceScene;
    const location = Array.isArray(companion.locationIds) && companion.locationIds.length
      ? adventureLocationById(companion.locationIds[0])
      : null;
    return (location && location.scene) || "草地小径";
  }
  ...
  function selectCompanion(companionId) {
    if (!companionById(companionId)) return;
  ...
  function openCompanionGiftShop(companionId) {
    ...
    setModal({ type: "companionGiftShop", companionId: companionId, page: 0 });
  }
  ...
  function buyGiftForCompanion(companionId, itemId) {
    ...
    record.knownFavorite = favoriteGiftIds.length > 0 && record.knownFavoriteIds.length >= favoriteGiftIds.length;
    record.knownDislike = dislikedGiftIds.length > 0 && record.knownDislikedIds.length >= dislikedGiftIds.length;
    state.companionGiftCount = Math.max(0, Number(state.companionGiftCount) || 0) + 1;
  ...
  function unlockAchievement(id, options) {
    ...
    if (ach && !(options && options.silent)) {
      queueToast("解锁成就：「" + ach.name + "」", "success");
    }
    return true;
  }

  function syncSocialAchievements(options) {
    let changed = false;
    const companionCount = unlockedCompanionCount();
    const level3CompanionCount = companionsAtLeastAffectionLevel(3);
    const giftCount = Math.max(0, Number(state.companionGiftCount) || 0);
    const hatchedCount = activeHatchedPetCount();
    if (hatchedCount >= 1) changed = unlockAchievement("adventure_pet_hatched", options) || changed;
    if (hasAdultPet()) changed = unlockAchievement("adventure_pet_adult", options) || changed;
    if (companionCount >= 1) changed = unlockAchievement("social_first_companion", options) || changed;
    if (companionCount >= 5) changed = unlockAchievement("social_companion_5", options) || changed;
    if (giftCount >= 3) changed = unlockAchievement("social_gift_3", options) || changed;
    if (level3CompanionCount >= 1) changed = unlockAchievement("social_affection_lv3_1", options) || changed;
    if (level3CompanionCount >= 5) changed = unlockAchievement("social_affection_lv3_5", options) || changed;
    return changed;
  }
  ...
  function registerClippedRegion(target, clipX, clipY, clipW, clipH, x, y, width, height, onClick, options) {
...
function drawPetBodyCorePixels(sctx, pet, size) {
  ...
  function getCompanionPortraitSprite(companion, size) {
    const safeSize = size || 32;
    if (!companion) return getTreasureSprite(safeSize);
    return getCompanionSceneSprite(companion, companionPortraitSceneName(companion), safeSize);
  }

  function getCompanionCodexSprite(companion, size) {
    return getCompanionPortraitSprite(companion, size || 32);
  }

  function getCompanionSceneSprite(companion, sceneName, size) {
    const safeSize = size || 32;
    return spriteCanvas("companion-scene:" + (companion ? companion.id : "none") + ":" + sceneName + ":" + safeSize, safeSize, function (sctx, canvasSize) {
      drawScenePixels(sctx, sceneName || "草地小径", canvasSize);
...
function getTreasureSprite(size) {
  ...
  function bucketSectionHeight(tasks) {
  ...
  function drawInputField(targetCtx, x, y, width, height, label, value, placeholder, fieldKey, numeric) {
    ...
    if (targetCtx === overlayCtx && overlayClipRect) {
      registerClippedRegion(overlayRegions, overlayClipRect.x, overlayClipRect.y, overlayClipRect.w, overlayClipRect.h, x, y + 24, width, height, function () { activateInput(fieldKey, numeric); }, { keepInput: true });
    } else {
  ...
  function drawSelectorField(targetCtx, x, y, width, height, label, value, onClick) {
    ...
    if (onClick) {
      ...
      if (targetCtx === overlayCtx && overlayClipRect) {
        registerClippedRegion(overlayRegions, overlayClipRect.x, overlayClipRect.y, overlayClipRect.w, overlayClipRect.h, x, y + 24, width, height, onClick);
      } else {
  ...
  function renderRest(targetCtx, x, y, width) {
    ...
    if (!hasSupplies) {
      drawCard(targetCtx, shellX + 18, cursor, shellW - 36, 72, THEME.paper);
      drawText(targetCtx, "这里还没有补给，去商店、冒险、Boss 或伙伴回礼里收一点吧", shellX + shellW / 2, cursor + 22, 22, THEME.inkSoft, "500", "center");
      return y + sectionHeight;
    }
    supplies.forEach(function (entry) {
      ...
      drawText(targetCtx, item.name + "  •  x" + count, textX, cursor + 26, 28, THEME.ink, "700");
      drawTextFitted(targetCtx, "恢复 " + (item.hp || 0) + " HP  •  售价 " + item.price + " 金币  •  稀有度 " + itemRarityLabel(item), textX, cursor + 66, Math.max(240, shellW - 420), 20, THEME.inkSoft, "500", "left", 14);
      drawParagraph(targetCtx, item.description + " 卖掉可得 " + supplySellPrice(item) + " 金币。", textX, cursor + 88, Math.max(220, shellW - 420), 16, THEME.inkSoft, 18, { maxLines: 2 });
  ...
  function renderTasksPage(targetCtx, view, y) {
    ...
    const tabBarH = 106;
    const challengeContentH = bucketSectionHeight(visibleTodo) + 16 + bucketSectionHeight(groups.habit) + 16 + bucketSectionHeight(groups.learn) + 40;
    ...
    drawSectionHeader(targetCtx, x + 30, y + 30, "任务区", tab === "challenge" ? "挑战列表" : "自定义奖励与已有补给");
  ...
  function renderCompanionPage(targetCtx, view, y) {
    ...
    drawSectionHeader(targetCtx, x + 30, y + 30, "伙伴档案", "在旅途中结识的伙伴");
    const listX = x + 28;
    const listY = y + 112;
    const listW = Math.min(332, Math.max(280, Math.round(w * 0.28)));
    const detailX = listX + listW + 20;
    const detailW = w - 56 - listW - 20;
    const bodyH = panelHeight - 140;
    drawCard(targetCtx, listX, listY, listW, bodyH, THEME.paper);
    drawCard(targetCtx, detailX, listY, detailW, bodyH, THEME.paper);
    ...
    const cardX = listX + 16;
    const cardW = listW - 44;
    const cardH = 112;
    const cardGap = 14;
    const listTopPadding = 18;
    const listBottomPadding = 18;
    const contentHeight = listTopPadding + COMPANIONS.length * (cardH + cardGap) - cardGap + listBottomPadding;
    const maxListScroll = Math.max(0, contentHeight - listViewport.height);
    const listScroll = clamp(Number(state.companionListScroll) || 0, 0, maxListScroll);
    state.companionListScroll = listScroll;
    companionListViewport = { x: listX, y: listY, width: listW, height: bodyH, maxScroll: maxListScroll };

    const selected = selectedCompanion();
    let cardY = listY + listTopPadding - listScroll;
    targetCtx.save();
    targetCtx.beginPath();
    targetCtx.rect(listViewport.x, listViewport.y, listViewport.width, listViewport.height);
    targetCtx.clip();
    COMPANIONS.forEach(function (companion) {
      const record = activeCompanionRecord(companion.id);
      const unlocked = record.unlocked;
      const active = selected && selected.id === companion.id;
      const visibleTop = Math.max(cardY, listViewport.y);
      const visibleBottom = Math.min(cardY + cardH, listViewport.y + listViewport.height);
      if (visibleBottom > visibleTop) {
        drawCard(targetCtx, cardX, cardY, cardW, cardH, active ? THEME.paperSoft : THEME.paper);
        drawSpriteFrame(
          targetCtx,
          unlocked ? getCompanionPortraitSprite(companion, 72) : getTreasureSprite(72),
          cardX + 14,
          cardY + 18,
          76
        );
        drawTextFitted(targetCtx, unlocked ? companion.name : "？？？伙伴", cardX + 108, cardY + 20, cardW - 126, 24, THEME.ink, "700", "left", 16);
        drawTextFitted(
          targetCtx,
          unlocked ? ("见面 " + record.meetings + " 次") : "等待在冒险中邂逅",
          cardX + 108,
          cardY + 56,
          cardW - 126,
          18,
        ...
        drawTextFitted(
          targetCtx,
          unlocked ? ("好感 Lv." + affinityMeta.level + " · " + affinityMeta.name) : "尚未解锁",
          cardX + 108,
          cardY + 84,
          cardW - 126,
          18,
          active ? THEME.ink : THEME.inkSoft,
          "600",
          "left",
          12
        );
        registerRegion(mainRegions, cardX, visibleTop, cardW, visibleBottom - visibleTop, function () { selectCompanion(companion.id); });
      }
      cardY += cardH + cardGap;
    });
    targetCtx.restore();
    if (maxListScroll > 0) {
      const trackX = listX + listW - 18;
      const trackY = listViewport.y + 6;
      const trackH = listViewport.height - 12;
      const thumbH = Math.max(68, trackH * (listViewport.height / contentHeight));
      const thumbY = trackY + ((trackH - thumbH) * (listScroll / maxListScroll));
      fillRect(targetCtx, trackX, trackY, 8, trackH, THEME.paperSoft);
      fillRect(targetCtx, trackX, thumbY, 8, thumbH, THEME.yellow);
      targetCtx.strokeStyle = THEME.frame;
      targetCtx.lineWidth = 2;
      targetCtx.strokeRect(trackX, thumbY, 8, thumbH);
    }
    ...
    const detailSprite = unlocked
      ? getCompanionPortraitSprite(selected, 120)
      : getTreasureSprite(120);
    drawSpriteFrame(targetCtx, detailSprite, detailX + 26, listY + 28, 220);
    drawTextFitted(targetCtx, unlocked ? selected.name : "？？？伙伴", detailX + 272, listY + 36, detailW - 298, 30, THEME.ink, "700", "left", 18);
    if (!unlocked) {
      drawParagraph(targetCtx, "这位伙伴还没有正式认识你。去冒险页对应的场景多走走，也许下一次就会遇见。", detailX + 272, listY + 94, detailW - 304, 18, THEME.inkSoft, 24, { maxLines: 4 });
      drawTextFitted(targetCtx, "解锁后会显示完整介绍、出没地点与礼物偏好。", detailX + 272, listY + 198, detailW - 304, 18, THEME.inkSoft, "500", "left", 12);
      drawButton(targetCtx, detailX + detailW - 182, listY + bodyH - 86, 146, 56, "送礼", "disabled", { small: true, disabled: true });
      return y + panelHeight;
    }

    drawParagraph(targetCtx, selected.intro, detailX + 272, listY + 88, detailW - 304, 18, THEME.inkSoft, 24, { maxLines: 4 });
    const affinityMeta = companionAffinityMeta(record.affection);
    drawText(targetCtx, "好感 Lv." + affinityMeta.level + " · " + affinityMeta.name, detailX + 26, listY + 262, 24, THEME.ink, "700");
    drawMeter(targetCtx, detailX + 26, listY + 330, detailW - 52, affinityMeta.progressValue, affinityMeta.progressMax, [THEME.expA, THEME.expB], "好感");

    let metaY = listY + 384;
    function drawMetaRow(label, value) {
      drawText(targetCtx, label + "：", detailX + 26, metaY, 16, THEME.inkSoft, "600");
      drawParagraph(targetCtx, value || "—", detailX + 126, metaY - 4, detailW - 164, 16, THEME.inkSoft, 20, { maxLines: 2 });
      metaY += 42;
    }
    drawMetaRow("出没地点", companionDisplayLocationNames(selected, record.knownLocationIds));
    drawMetaRow("喜欢的礼物", companionDisplayGiftName(companionFavoriteGiftIds(selected), record.knownFavoriteIds));
    drawMetaRow("讨厌的礼物", companionDisplayGiftName(companionDislikedGiftIds(selected), record.knownDislikedIds));
    drawMetaRow("见面次数", String(record.meetings) + " 次");
    drawMetaRow("上次见面", formatRelativeTimeFromNow(record.lastMetAt, "还没见过面"));

    drawButton(targetCtx, detailX + detailW - 182, listY + bodyH - 74, 146, 56, "送礼", "green", { small: true });
    registerRegion(mainRegions, detailX + detailW - 182, listY + bodyH - 74, 146, 56, function () { openCompanionGiftShop(selected.id); });
    return y + panelHeight;
  }
  ...
  function renderSocialPage(targetCtx, view, y) {
    ...
    drawSectionHeader(targetCtx, x + 30, y + 30, "社交页", tab === SOCIAL_TABS.pets ? "宠物与伙伴" : "伙伴与宠物");
  ...
  function renderAdventurePage(targetCtx, view, y) {
    ...
    const trip = currentAdventureTrip();
    if (trip) {
      const actionButtonW = 168;
      const actionButtonGap = 14;
      const actionButtonY = progressY + 178;
      const statusButtonX = x + w - 250;
      const remaining = Math.max(0, Math.ceil((trip.endsAt - Date.now()) / 1000));
      const ready = remaining === 0;
      const infoWidth = ready
        ? (w - 340)
        : Math.max(200, statusButtonX - actionButtonW - actionButtonGap - (x + 54) - 24);
      drawTextFitted(targetCtx, ready ? "当前冒险已结束，可以领取战利品。" : "正在探索： " + trip.locationName, x + 54, progressY + 192, infoWidth, 20, THEME.ink, "600", "left", 14);
      drawTextFitted(targetCtx, ready ? "战利品已经准备好了。" : "剩余时间： " + formatClock(remaining), x + 54, progressY + 222, infoWidth, 18, THEME.inkSoft, "500", "left", 14);
      if (!ready) {
        const skipCost = adventureSkipCost(trip);
        const skipButtonX = statusButtonX - actionButtonW - actionButtonGap;
        const canSkip = state.profile.gold >= skipCost;
        drawButton(targetCtx, skipButtonX, actionButtonY, actionButtonW, 64, "跳过（" + skipCost + "金币）", canSkip ? "yellow" : "disabled", { small: true, disabled: !canSkip });
        if (canSkip) registerRegion(mainRegions, skipButtonX, actionButtonY, actionButtonW, 64, skipAdventure);
      }
      drawButton(targetCtx, statusButtonX, actionButtonY, actionButtonW, 64, ready ? "领取战利品" : "冒险进行中", ready ? "yellow" : "disabled", { small: true, disabled: !ready });
      if (ready) registerRegion(mainRegions, statusButtonX, actionButtonY, actionButtonW, 64, collectAdventure);
    } else {
  ...
  function renderCompanionGiftShopModal(targetCtx, view) {
    ...
    pageItems.forEach(function (item, index) {
      ...
      drawCard(targetCtx, cardX, cardY, cardW, 150, THEME.paper);
  ...
  function renderCodexModal(targetCtx, view) {
    ...
    filtered.forEach(function (entry, index) {
      ...
      registerClippedRegion(overlayRegions, leftX, leftY, leftWInner, leftHInner, cx, cy, cardW, cardH, function () {
  ...
  function renderSettingsModal(targetCtx, view) {
    ...
    const baseHeight = activeTab === "theme" ? 190 + rows * cardH + (rows - 1) * gap : 190 + 148 + 130 + 130 + 2 * 18;
  ...
  function renderEditorModal(targetCtx, view) {
    ...
    overlayClipRect = { x: viewportX, y: viewportY, w: viewportW, h: viewportH };
    ...
    rowY += 108;
    drawEditorChoiceRow(targetCtx, controlX, rowY + contentShift, controlW, "accessories", "已拥有配饰", state.collection.accessories.map(function (value) { return { label: value, width: compactEditor ? 120 : 170, active: value === "无" ? (currentAccessories(state.profile.appearance).length === 0 || currentAccessories(state.profile.appearance).includes("无")) : currentAccessories(state.profile.appearance).includes(value), onClick: function () { equipAppearanceFromCollection("accessory", value); } }; }));
    targetCtx.restore();
  ...
  function handleMainCanvasWheel(event) {
    if (!currentView || state.modal || !companionListViewport) return;
    if (state.page !== "pets" || state.socialTab !== SOCIAL_TABS.companions) return;
    const point = canvasPoint(event, mainCanvas, currentView);
    if (!pointInRect(point, companionListViewport)) return;
    const maxScroll = Math.max(0, companionListViewport.maxScroll || 0);
    if (maxScroll <= 0) return;
    event.preventDefault();
    const delta = event.deltaY > 0 ? 72 : -72;
    const current = clamp(Number(state.companionListScroll) || 0, 0, maxScroll);
    const next = clamp(current + delta, 0, maxScroll);
    if (next === current) return;
    state.companionListScroll = next;
    renderApp();
  }
  ...
  mainCanvas.addEventListener("wheel", handleMainCanvasWheel, { passive: false });
